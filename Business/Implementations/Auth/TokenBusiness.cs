...
namespace Business.Custom
{
    public class TokenBusiness : ITokenBusiness
    {
        ...
        private readonly IRefreshTokenData _refreshRepo;
        private readonly JwtSettings _jwtSettings;
        public TokenBusiness(IConfiguration configuration, IUserBusiness userBusiness, ILogger<TokenBusiness> logger, IRefreshTokenData refreshToken, IOptions<JwtSettings> jwtSettings)
        {
        ...
        /// </summary>
        public async Task<(string AccessToken, string RefreshToken, string CsrfToken)> GenerateTokensAsync(LoginRequestDto dto)
        {
            UserDto user = await _userBusiness.LoginUser(dto);
        ...
        public async Task<(string NewAccessToken, string NewRefreshToken)> RefreshAsync(string refreshTokenPlain, string remoteIp = null)
        {
            ...
            UserDto user = await _userBusiness.GetById(record.UserId)
                ?? throw new SecurityTokenException("Usuario no encontrado");


            // 1) Nuevo access token
            var newAccessToken = BuildAccessToken(user);
        ...
        /// </summary>
        private string BuildAccessToken(UserDto user)
        {
            var now = DateTime.UtcNow;
